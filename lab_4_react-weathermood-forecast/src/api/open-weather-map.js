...
const key = '40fae0d579e36ebc0bb5e40ae3b9617b';
const baseUrl = `http://api.openweathermap.org/data/2.5/weather?appid=${key}`;
const baseForUrl = `http://api.openweathermap.org/data/2.5/forecast?&appid=${key}`;
...
export function getForecast(city, unit) {
    var url = `${baseForUrl}&q=${encodeURIComponent(city)}&units=${unit}`;
    ...
    return axios.get(url, {cancelToken: weatherSource.token}).then(function(res) {
        if (res.data.cod && res.data.message) {
            ...
            for (let i=0; i<40; i+=8) {
                ...
                const t = {
                    code: l.weather[0].id,
                    group: getWeatherGroup(l.weather[0].id),
